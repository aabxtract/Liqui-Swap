...
import { INITIAL_POOLS, INITIAL_USER_BALANCE, SWAP_FEE, TOKENS } from '@/lib/constants';
...
const liquidityReducer = (state: State, action: Action): State => {
  ...
  switch (action.type) {
    case 'SWAP': {
      ...
      toast({ title: "Swap Successful", description: `Swapped ${fromAmount.toFixed(4)} ${fromToken.symbol} for ${amountOut.toFixed(4)} ${toToken.symbol}` });
    ...
    case 'ADD_LIQUIDITY': {
        ...
        toast({ title: "Liquidity Added", description: `Received ${lpTokensToMint.toFixed(4)} LP tokens.` });
        return { ...state, balances: newBalances, pools: newPools };
    }
    case 'REMOVE_LIQUIDITY': {
        ...
        toast({ title: "Liquidity Removed", description: `Received ${amount0.toFixed(4)} ${pool.token0.symbol} and ${amount1.toFixed(4)} ${pool.token1.symbol}` });
        return { ...state, balances: newBalances, pools: newPools };
    }
    case 'SET_SLIPPAGE': {
